...
export default function Home() {
  ...
  useEffect(() => {
    ...
    // Periodically trigger backend weather checks every 10 minutes
  useEffect(() => {
  // Run once when page loads
  fetch("/api/checkWeather");
  console.log("Initial weather check triggered");

  // Then check every 3 minutes while page is open
  const interval = setInterval(() => {
    fetch("/api/checkWeather");
    console.log("Weather check triggered (every 3 mins)");
  }, 3 * 60 * 1000); // 3 minutes
            ...
            <section className="flex w-full flex-col gap-4  ">
              ...
              {firstDataForEachDate.map((d, i) => (
                <ForecastWeatherDetail
                  ...
                  day={d ? format(parseISO(d.dt_txt), "dd.MM") : "03.11"}
